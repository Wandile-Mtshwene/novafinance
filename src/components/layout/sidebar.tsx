...
const navGroups: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Receivables", href: "/dashboard/receivables", icon: TrendingUp },
    ],
  },
  {
    label: "Money Out",
    items: [
      ...
      { label: "Payables", href: "/dashboard/payables", icon: TrendingDown },
    ],
  },
  {
    ...
    items: [
      ...
      { label: "Projections", href: "/dashboard/projections", icon: BarChart3 },
    ],
  },
  {
    label: "Reporting",
    items: [
      { label: "Statements", href: "/dashboard/statements", icon: BookOpen },
      { label: "Reports", href: "/dashboard/reports", icon: Scale },
      { label: "Compliance", href: "/dashboard/compliance", icon: ShieldCheck },
    ],
  },
];

type UserRole = "owner" | "admin" | "accountant" | "finance_manager" | "bookkeeper" | "viewer";
...
export function Sidebar({ user, role }: SidebarProps) {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("novafinance-sidebar");
  ...
  function toggleCollapsed() {
    ...
    setCollapsed(next);
    localStorage.setItem("novafinance-sidebar", next ? "collapsed" : "expanded");
  }
    ...
    >
      ...
      {collapsed ? (
        <div className="flex flex-col items-center gap-1.5 border-b border-[var(--nova-border)] py-3 px-2">
          <Link href="/dashboard" aria-label="NovaFinance home">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Icon
                        ...
                        className={cn(
                          "shrink-0",
                          active ? "text-[var(--nova-accent)]" : "text-[var(--nova-muted)] group-hover:text-[var(--nova-text)]"
                        )}
                      />
                      {item.label}
                    </div>
      ...
      <div className="border-t border-[var(--nova-border)]">
        {collapsed ? (
          ...
          <div className="p-2 space-y-0.5">
            ...
            >
              <Settings size={17} className={cn("shrink-0", isActive("/dashboard/settings") ? "text-[var(--nova-accent)]" : "text-[var(--nova-muted)] group-hover:text-[var(--nova-text)]")} />
              Settings
            </Link>
            <div className="flex items-center gap-2.5 rounded-lg px-3 py-2">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  <p className="text-sm font-medium text-[var(--nova-text)] truncate leading-none">{displayName}</p>
                ...
                {user.name && (
                  <p className="mt-0.5 text-[11px] text-[var(--nova-muted)] truncate">{user.email}</p>
                )}
              </div>
              <button
                onClick={handleSignOut}
